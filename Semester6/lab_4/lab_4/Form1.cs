...
namespace lab_4
{
    public partial class Form1 : Form
    {
        ...
        {
            InitializeComponent();
            mainPlot.Plot.Axes.SetLimitsX(-EPSILON - 0.1, 1.1 + EPSILON);
            mainPlot.Plot.Axes.SetLimitsY(-EPSILON - 0.1, 0.5 + EPSILON);
        }
        ...
        private void Main()
        {
            ...
            DrawLine(x, u, "tg(x^3 / 3)", ScottPlot.Color.FromColor(Color.Black));
            DrawLine(x, y1_1, "Трапеция", ScottPlot.Color.FromColor(Color.Red));
            DrawLine(x, y2_1, "Рунги-Кутты", ScottPlot.Color.FromColor(Color.Green));
            DrawLine(x, y3_1, "Адамс", ScottPlot.Color.FromColor(Color.Blue));
        }

        private void DrawLine(double[] xs, double[] ys, string lineName, ScottPlot.Color color)
        {
            var line = mainPlot.Plot.Add.SignalXY(xs, ys, color);
            line.LegendText = lineName;
            mainPlot.Refresh();
        }
